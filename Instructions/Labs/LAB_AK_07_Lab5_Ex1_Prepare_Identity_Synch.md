...
21. In the query results, note how two of the three users who you just fixed no longer appear in the results (Holly and Klemen). The exception is Maj Hoski. When you originally broke Maj's account by running the script in the prior task, it replaced her email address with blank characters. Then when you flagged her account to be edited back in Step 13, the IdFix tool replaced the blank characters with Maj's name. Now you need to fix this value by replacing her name with her actual email address. <br/>
